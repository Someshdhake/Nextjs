...
const Checkout = ({cart, subTotal, addToCart, removeFromCart}) => {
  return (
    <div className='container m-auto'>
      ...
      <h2 className='mx-36 font-semiboldbold text-xl'>1. Delivery Datails</h2>
      <div className="mx-auto flex my-2">
        <div className="px-2 w-1/2 ml-36">
          <div className="mb-4">
            <label htmlFor="name" className="leading-7 text-sm text-gray-600">Name</label>
            <input type="text" id="name" name="name" className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
          </div>
        </div>
        <div className="px-2 w-1/2 mr-36">
          <div className="mb-4">
      ...
      <div className="px-2 w-full mx-36">
        <div className="mb-4 mr-72">
      ...
      <h2 className='mx-36 font-semiboldbold text-xl'>2. Review Cart Items & Pay</h2>
      <div className="  sideCart bg-indigo-200 p-6 m-2 mx-36">
          ...
          {Object.keys(cart).map((k) => {
            return <li key={k}>
              <div className="item flex">
                ...
                <div className='flex font-semibold items-center justify-center w-1/3 text-lg '><AiFillMinusCircle onClick={() => { removeFromCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].varient) }} className='cursor-pointer text-indigo-500' /> <span className="mx-2 text-sm">{cart[k].qty}</span> <AiFillPlusCircle onClick={()=>{addToCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].varient)}} className='cursor-pointer text-indigo-500' /></div>
              </div>
            </li>
          })}
        </ol>
      ...
      <div className="mx-36">
      <Link href={"/checkout"}><button className="flex text-white bg-indigo-500 border-0 py-2 px-2 focus:outline-none hover:bg-indigo-600 rounded text-sm"><BsFillBagCheckFill className='m-1' />Pay ₹ {subTotal}</button></Link>
      </div>
    </div>
